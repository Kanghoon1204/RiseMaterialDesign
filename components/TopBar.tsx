      ...
      <div className="flex items-center justify-between h-full px-4">
        <div className="flex items-center space-x-4">
          ...
          <Link to="/home" className="flex items-center space-x-2">
             ...
             <span className="text-xl font-medium text-neutral-800 dark:text-neutral-100">{t.common.materialDesign}</span>
          </Link>
        </div>
        <div className="flex items-center space-x-2 sm:space-x-4">
          <div className="hidden md:flex items-center space-x-2 px-3 py-2 rounded-full bg-slate-200 dark:bg-neutral-700 w-64">
             ...
             <input type="text" placeholder={t.common.search} className="bg-transparent focus:outline-none w-full text-sm"/>
          </div>
          <button
            onClick={toggleLanguage}
            className="px-3 py-1.5 rounded-full text-sm font-medium bg-slate-200 dark:bg-neutral-700 hover:bg-slate-300 dark:hover:bg-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
            aria-label="Toggle language"
          >
            {language === 'ko' ? 'KO' : 'EN'}
          </button>
          <button onClick={toggleTheme} className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-neutral-700 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label={t.common.toggleTheme}>
            {theme === 'light' ? (
